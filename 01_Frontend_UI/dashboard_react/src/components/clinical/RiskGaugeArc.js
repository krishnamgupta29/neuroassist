...
export default function RiskGaugeArc({
  ...
  return (
    <div className="clinical-card p-5 bg-white flex flex-col justify-between">
      ...
      {/* Probabilities Distribution breakdown with animated horizontal progress bars */}
      <div className="mt-5 pt-4 border-t border-[#F0EBE5] space-y-3">
              ...
              <div key={idx} className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1.5 font-bold" style={{ color: item.color }}>
                    ...
                    />
                    <span className="text-[#22201F] text-xs font-semibold">{item.name}</span>
                  </div>
                  <span className="font-mono text-xs font-bold" style={{ color: item.color }}>
                    {round1(item.val)}%
                  </span>
                </div>
                {/* Track and Animated Bar */}
                <div className="w-full h-3 rounded-full bg-[#F0E8E1] overflow-hidden p-0.5 relative shadow-inner">
                  <div
        ...
        <div className="grid grid-cols-3 gap-2 pt-2">
          {chartData.map((item, idx) => (
            <div
              key={idx}
              className="p-2 rounded-xl border border-[#E8E2DA] text-center transition-all hover:shadow-clinical-xs"
